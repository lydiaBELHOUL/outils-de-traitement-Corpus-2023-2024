...
# -*- coding: utf-8 -*-
"""
calcul des correlation entre la moyenne du nombre de tockens dans le tableau, phrase1, paragraphe
visualistion du résultat de la coorelation
...
download('punkt')  # Assurez-vous que le tokenizer NLTK est téléchargé.
...
correlation = np.corrcoef(tokens_table, tokens_context)[0, 1]
...
table.add_row(["Moyenne des tokens par contexte", f"{average_tokens_context:.2f}"])
table.add_row(["Corrélation Table-Contexte", f"{correlation:.2f}"])
...
plt.bar(['Tokens par tableau', 'Tokens par première phrase', 'Tokens par contexte'], [average_tokens_table, average_tokens_first_sentence, average_tokens_context], color=['blue', 'green', 'red'])
